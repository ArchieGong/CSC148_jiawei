...
class TMTree:
    ...
    def __init__(self, name: str, subtrees: list[TMTree],
        ...
        self._name = name
        self.rect = None
        self._subtrees = subtrees.copy()
        self._parent_tree = None
        self._expanded = False
        self._colour = (randint(0, 256), randint(0, 256), randint(0, 256))
        if subtrees is None or name is None:
    ...
    def is_displayed_tree_leaf(self) -> bool:
        ...
        False
        """
        return not bool(self._subtrees)
    ...
    def update_rectangles(self, rect: tuple[int, int, int, int]) -> None:
        ...
        if not self._subtrees:
            ...
            total_width, sub_rect = 0, []
            for subtree in self._subtrees:
                sub_width = math.trunc(subtree.data_size*width/total_size)
                sub_height = height
                sub_rect.append((x+total_width, y, sub_width, sub_height))
                total_width += sub_width
            for i, subtree in enumerate(self._subtrees):
                subtree.update_rectangles(sub_rect[i])
        else:
            total_height, sub_rect = 0, []
            for subtree in self._subtrees:
                sub_width = width
                sub_height = math.trunc(subtree.data_size*height/total_size)
                sub_rect.append((x, y+total_height, sub_width, sub_height))
    ...
    tuple[int, int, int]]]:
        ...
        if self.data_size == 0:
            ...
            rectangles = []
            if not self._subtrees:
                rectangles.append((self.rect, self._colour))
            elif self._expanded:
                for subtree in self._subtrees:
    ...
    def expand_all(self) -> TMTree:
        ...
        last_ = None
        for subtree in self._subtrees:
            if not subtree.is_displayed_tree_leaf():
                subtree.expand()
                last_ = subtree.expand_all()
            else:
                last_ = subtree
        return last_
    ...
    def collapse(self) -> TMTree:
        ...
        if not self._parent_tree:
            return self
        index_ = self._parent_tree._subtrees.index(self)
        self._parent_tree._subtrees.pop(index_)
        self._parent_tree._expanded = False
        curr = self._parent_tree
        while curr and not curr._expanded:
            curr = curr._parent_tree
        return curr or self._parent_tree
    ...
    def collapse_all(self) -> TMTree:
        ...
        parent._subtrees = [self]
        self._parent_tree = parent
        parent._expanded = True
        for subtree in self._subtrees:
            subtree._parent_tree = None
        return parent
