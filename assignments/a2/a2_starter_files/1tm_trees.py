...
def dir_tree_from_nested_tuple(obj: tuple[str, int | list]) -> DirectoryTree:
    ...
    if tuple[1] is int:
        ...
        subtree = []
        for item in obj[1]:
            subtree.append(dir_tree_from_nested_tuple(item))
...
class FileTree:
    ...
    def __init__(self, _name: str, data_size: int) -> None:
        super().__init__(_name, data_size)
...
class DirectoryTree:
    ...
    def __init__(self, _name: str, subtrees: list[TMTree] = []) -> None:
        super().__init__(_name, subtrees)

    def __str__(self, string: str = "", indent: int = 0) -> str:
        tab = "    "
        if string == "":
            rsl = f"{indent * tab}{self._name}/"f"({self.data_size}){self.rect}"
        for subtree in self._subtrees:
            ...
            if subtree._subtrees:
                rsl = DirectoryTree.__str__(subtree, string, indent + 1)
        rsl.replace("/", os.path.sep)
        return rsl
