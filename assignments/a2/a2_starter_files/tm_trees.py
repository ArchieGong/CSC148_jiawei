...
class TMTree:
        ...
        if subtrees:
            ...
            if rect[2] > rect[3]:
                ...
                for s in subtrees[:-1]:
                    ...
                    new_x += new_width
                subtrees[-1].update_rectangles((new_x, rect[1],
                                                rect[0] + rect[2] - new_x,
                                                rect[3]))
            else:
                new_y = rect[1]
                for s in subtrees[:-1]:
                    ...
                    new_y += new_height
                subtrees[-1].update_rectangles((rect[0], new_y, rect[2],
                                                rect[1] + rect[3] - new_y))
    ...
    def _get_tree_at_position_helper(self, pos: tuple[int, int]) -> \
            Optional[TMTree]:
        """
        A helper function that recursively searches for the leaf that contains
        the given position. If a leaf contains the position and is not expanded,
        it returns that leaf, otherwise it return None
        """
        if self._subtrees:
            for s in self._subtrees:
    ...
    def move(self, destination: TMTree) -> None:
        ...
        """
        node = self
        original_parent = node._parent_tree
        original_parent._expanded = len(original_parent._subtrees) > 1
        parent_tree = original_parent
        while parent_tree is not None:
            parent_tree.data_size -= node.data_size
            parent_tree = parent_tree._parent_tree

        original_parent._subtrees.remove(node)
        destination._subtrees.append(node)
        node._parent_tree = destination
    ...
    def change_size(self, factor: float) -> None:
        ...
        (0, 100, 100, 100)
        """
        changed = round(self.data_size * factor)
        self.data_size += changed

        subtree_total_size = 0
        for subtree in self._subtrees:
            subtree_total_size += subtree.data_size
        if self.data_size < subtree_total_size:
            self.data_size = subtree_total_size
        if self.data_size < 1:
            self.data_size = 1
...
class DirectoryTree(TMTree):
    ...
    def calculate_data_size(self) -> int:
        """
        Return the total size of all files and directories.
        """
