...
class Criterion:
    ...
    def score_answers(self, question: Question, answers: list[Answer]) -> float:
        ...
        score = len(valid_answers) / len(answers)
        return score
...
class LonelyMemberCriterion:
    ...
    def score_answers(self, question: Question, answers: list[Answer]) -> float:
        ...
        """
        for a in answers:
            if a.is_valid(question) is not True:
                raise InvalidAnswerError
        answer_include = [a.content for a in answers]
        for a in answer_include:
            unique_answer = answer_include.count(a)
            if unique_answer == 1:
                return 0.0
        else:
            return 1.0
