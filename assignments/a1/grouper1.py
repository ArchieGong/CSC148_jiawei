...
class AlphaGrouper(Grouper):
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        """
        sorted_student = sort_students(list(course.get_students()), 'name')
        sliced_list = slice_list(sorted_student, self.group_size)
        alpha = Grouping()
        for group in sliced_list:
            temp_group = Group(group)
            alpha.add_group(temp_group)
        return alpha
...
class GreedyGrouper(Grouper):
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        Preconditions:
            - <course> has more students than this Grouper's group_size
        """
        id_students = sort_students(list(course.get_students()), 'id')
        greedy = Grouping()
        while len(id_students) > 0:
            group = []
            first_stu = id_students.pop(0)
            group.append(first_stu)
            while len(group) < self.group_size and len(id_students) > 0:
                best = find_best_addition_to_group(survey, group, id_students)
                id_students.remove(best)
                group.append(best)
            new = Group(group)
            greedy.add_group(new)
        return greedy
...
class SimulatedAnnealingGrouper(Grouper):
    ...
    def __init__(self,
        ...
        self._initial_t = initial_temperature
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        cur_score = total_score(survey, best_grouping)
        cur_temp = self._initial_t
        sa = Grouping()

        for i in range(self._iteration):
            random_swap(sliced_stu, seed=i)
            new_score = total_score(survey, sliced_stu)
            if accept(cur_score, new_score, cur_temp, seed=i):
                if new_score > cur_score:
                    best_grouping = copy.deepcopy(sliced_stu)
                cur_score = new_score
